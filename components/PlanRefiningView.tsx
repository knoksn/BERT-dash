...
interface PlanRefiningViewProps {
  onRefiningComplete: () => void;
}

const PlanRefiningView: React.FC<PlanRefiningViewProps> = ({ onRefiningComplete }) => {
  ...
  useEffect(() => {
    ...
    if (isRefining && !isComplete) {
      let stepIndex = 0;
      interval = setInterval(() => {
        if (stepIndex < PLAN_REFINING_STEPS.length) {
          const currentStep = PLAN_REFINING_STEPS[stepIndex];
      ...
      <div className="bg-dark-card shadow-lg rounded-xl border border-dark-border overflow-hidden">
        <div className="p-6">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">Step 2: Refine Event Plan</h2>
              <p className="text-dark-text-secondary">Simulating the process of polishing ideas and logistics.</p>
            </div>
          </div>
        ...
        <div className="bg-gray-900/50 px-6 py-4 flex items-center justify-end gap-4">
          {!isRefining ? (
            ...
            >
              <GiftIcon className="w-5 h-5 animate-spin" />
              Refining...
            </button>
          )}
        </div>
...
export default PlanRefiningView;
