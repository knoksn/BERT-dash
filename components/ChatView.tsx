...
import { ChatMessage, FineTuningData } from '../types';
import { ChatBubbleIcon, SendIcon } from './shared/IconComponents';
...
const ChatView: React.FC<ChatViewProps> = ({ fineTuningData }) => {
  ...
  useEffect(() => {
    chatRef.current = createChatSession(fineTuningData);
    setMessages([
      {
        id: 'init',
        sender: 'bot',
        text: "DarkBERT model 'darkbert-v1-finetuned' is online. My knowledge has been updated with your data. I am ready for your queries.",
      },
    ]);
  }, [fineTuningData]);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
        ...
        // Handle cases where the stream ends with no text
        if (fullResponse.trim() === '') {
             setMessages(prev =>
                prev.map(msg =>
                  msg.id === botMessageId ? { ...msg, text: "I don't have a response for that." } : msg
                )
             );
        }
    ...
    <div className="w-full max-w-4xl mx-auto p-4 md:p-6 h-[calc(100vh-120px)] flex flex-col">
      <div className="bg-dark-card shadow-lg rounded-xl border border-dark-border flex flex-col flex-grow">
        <div className="p-4 border-b border-dark-border flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-white">Chat with DarkBERT</h2>
            <p className="text-dark-text-secondary">Model: darkbert-v1-finetuned</p>
          </div>
        </div>
            ...
            {messages.map((msg, index) => {
                ...
                return (
                  <div key={msg.id} className={`flex items-end gap-2 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                    {msg.sender === 'bot' && <div className="w-8 h-8 rounded-full bg-accent flex items-center justify-center font-bold text-gray-900 text-sm flex-shrink-0">DB</div>}
                    <div className={`max-w-md lg:max-w-xl px-4 py-2 rounded-2xl ${msg.sender === 'user' ? 'bg-blue-600 text-white rounded-br-none' : 'bg-dark-border text-dark-text rounded-bl-none'} flex items-center`}>
                      ...
                      {msg.text ? (
                          ...
                          // Show spinner inside the bubble if text is empty and we are waiting for a response
