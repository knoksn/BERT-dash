...
const WorldBuildingView: React.FC<WorldBuildingViewProps> = ({ onBuildingComplete }) => {
  ...
  useEffect(() => {
    ...
    if (isBuilding && !isComplete) {
      ...
      interval = setInterval(() => {
        if (stepIndex < WORLD_BUILDING_STEPS.length) {
          const currentStep = WORLD_BUILDING_STEPS[stepIndex];
      ...
      <div className="bg-dark-card shadow-lg rounded-xl border border-dark-border overflow-hidden">
        <div className="p-6">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">Step 2: World-Building</h2>
              <p className="text-dark-text-secondary">Simulating the creation of the quest environment.</p>
            </div>
          </div>
        ...
        <div className="bg-gray-900/50 px-6 py-4 flex items-center justify-end gap-4">
          {isComplete ? (
            ...
            >
              <ScrollIcon className="w-5 h-5 animate-spin" />
              Building...
            </button>
          )}
        </div>
