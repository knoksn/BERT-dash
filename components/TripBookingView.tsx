...
const TripBookingView: React.FC<TripBookingViewProps> = ({ onBookingComplete }) => {
  ...
  useEffect(() => {
    ...
    if (isBooking && !isComplete) {
      let stepIndex = 0;
      interval = setInterval(() => {
        if (stepIndex < TRIP_BOOKING_STEPS.length) {
          const currentStep = TRIP_BOOKING_STEPS[stepIndex];
      ...
      <div className="bg-dark-card shadow-lg rounded-xl border border-dark-border overflow-hidden">
        <div className="p-6">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">Step 2: Simulating Trip Booking</h2>
              <p className="text-dark-text-secondary">Simulating checking flights, hotels, and activities.</p>
            </div>
          </div>
        ...
        <div className="bg-gray-900/50 px-6 py-4 flex items-center justify-end gap-4">
          {isComplete ? (
            ...
            >
              <GlobeIcon className="w-5 h-5 animate-spin" />
              Booking...
            </button>
          )}
        </div>
