...
interface ArchiveIndexingViewProps {
  onIndexingComplete: () => void;
}

const ArchiveIndexingView: React.FC<ArchiveIndexingViewProps> = ({ onIndexingComplete }) => {
  ...
  useEffect(() => {
    ...
    if (isIndexing && !isComplete) {
      ...
      interval = setInterval(() => {
        if (stepIndex < ARCHIVE_INDEXING_STEPS.length) {
          const currentStep = ARCHIVE_INDEXING_STEPS[stepIndex];
      ...
      <div className="bg-dark-card shadow-lg rounded-xl border border-dark-border overflow-hidden">
        <div className="p-6">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">Step 2: Index Archives</h2>
              <p className="text-dark-text-secondary">Simulating the process of gathering and verifying historical sources.</p>
            </div>
          </div>
        ...
        <div className="bg-gray-900/50 px-6 py-4 flex items-center justify-end gap-4">
          {!isIndexing ? (
            ...
            >
              <BookOpenIcon className="w-5 h-5 animate-spin" />
              Indexing...
            </button>
          )}
        </div>
...
export default ArchiveIndexingView;
