...
import { ChatMessage, AnniversaryPlan } from '../types';
import { ChatBubbleIcon, SendIcon, GiftIcon } from './shared/IconComponents';
...
const InteractivePlannerView: React.FC<InteractivePlannerViewProps> = ({ anniversaryPlan }) => {
  ...
  useEffect(() => {
    if (anniversaryPlan) {
        chatRef.current = createAnniBertChatSession(anniversaryPlan);
        setMessages([
          {
            id: 'init',
            sender: 'bot',
            text: `Plan refined! I have the details for the ${anniversaryPlan.occasion}. Let's make it special! What should we brainstorm first?`,
          },
        ]);
    }
  }, [anniversaryPlan]);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (fullResponse.trim() === '') {
             setMessages(prev => prev.map(msg => msg.id === botMessageId ? { ...msg, text: "That's a great thought! How can we build on that?" } : msg));
        }
    } catch (error) {
        console.error("Error sending message:", error);
        setMessages(prev => prev.map(msg => msg.id === botMessageId ? { ...msg, text: 'Sorry, my creativity seems to be on a short break. Please try again.' } : msg));
    } finally {
        setIsThinking(false);
    }
  };
    ...
    <div className="w-full max-w-4xl mx-auto p-4 md:p-6 h-[calc(100vh-120px)] flex flex-col">
      <div className="bg-dark-card shadow-lg rounded-xl border border-dark-border flex flex-col flex-grow">
        <div className="p-4 border-b border-dark-border flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-white">Plan with AnniBERT</h2>
            <p className="text-dark-text-secondary">Occasion: {anniversaryPlan.occasion}</p>
          </div>
        </div>
            ...
            {messages.map((msg, index) => {
                ...
                return (
                  <div key={msg.id} className={`flex items-end gap-2 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                    {msg.sender === 'bot' && <div className="w-8 h-8 rounded-full bg-accent flex items-center justify-center font-bold text-gray-900 text-sm flex-shrink-0"><GiftIcon className="w-5 h-5"/></div>}
