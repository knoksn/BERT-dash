...
import { ChatMessage, ProductionPlan } from '../types';
import { ChatBubbleIcon, SendIcon, ClipboardListIcon } from './shared/IconComponents';
...
const ShowControlView: React.FC<ShowControlViewProps> = ({ productionPlan }) => {
  ...
  useEffect(() => {
    if (productionPlan) {
        chatRef.current = createLiveBertChatSession(productionPlan);
        const initialBotMessage: ChatMessage = {
          id: 'init',
          sender: 'bot',
          text: `Show Control, copy. I am on channel and have the production plan for "${productionPlan.eventTitle}". Ready for cues.`
        };
        // This check prevents re-adding the initial message on subsequent renders
        if (messages.length === 0) {
           setMessages([initialBotMessage]);
        }
    }
  }, [productionPlan, messages.length]);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (fullResponse.trim() === '') {
             setMessages(prev => prev.map(msg => msg.id === botMessageId ? { ...msg, text: "Show Control, say again?" } : msg));
        }
    } catch (error) {
        console.error("Error sending message:", error);
        setMessages(prev => prev.map(msg => msg.id === botMessageId ? { ...msg, text: 'Show Control, comms are fuzzy. Please try again.' } : msg));
    } finally {
        setIsThinking(false);
    }
  };
    ...
    <div className="w-full max-w-4xl mx-auto p-4 md:p-6 h-[calc(100vh-120px)] flex flex-col">
      <div className="bg-dark-card shadow-lg rounded-xl border border-dark-border flex flex-col flex-grow">
        <div className="p-4 border-b border-dark-border flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-white">Show Control with LiveBERT</h2>
            <p className="text-dark-text-secondary truncate pr-4">Event: {productionPlan.eventTitle}</p>
          </div>
        </div>
            ...
            {messages.map((msg, index) => {
                ...
                return (
                  <div key={msg.id} className={`flex items-end gap-2 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                    {msg.sender === 'bot' && <div className="w-8 h-8 rounded-full bg-accent flex items-center justify-center font-bold text-gray-900 text-sm flex-shrink-0"><ClipboardListIcon className="w-5 h-5"/></div>}
