...
import { ChatMessage, CaseBrief } from '../types';
...
const LegalQaView: React.FC<LegalQaViewProps> = ({ caseBrief }) => {
  ...
  useEffect(() => {
    if (caseBrief) {
        chatRef.current = createLaBertChatSession(caseBrief);
  ...
  useEffect(() => {
    const sendInitialMessage = async () => {
      if (chatRef.current && !isInitialMessageSent.current && messages.length === 0) {
        ...
        try {
          const stream = await chatRef.current.sendMessageStream({ message: "Let's begin." });
          let fullResponse = '';
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (fullResponse.trim() === '') {
             setMessages(prev => prev.map(msg => msg.id === botMessageId ? { ...msg, text: "That's an interesting question. Can you provide more context?" } : msg));
        }
    } catch (error) {
