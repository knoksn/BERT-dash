...
const LegalResearchView: React.FC<LegalResearchViewProps> = ({ onResearchComplete }) => {
  ...
  useEffect(() => {
    ...
    if (isResearching && !isComplete) {
      let stepIndex = 0;
      interval = setInterval(() => {
        if (stepIndex < LEGAL_RESEARCH_STEPS.length) {
          const currentStep = LEGAL_RESEARCH_STEPS[stepIndex];
      ...
      <div className="bg-dark-card shadow-lg rounded-xl border border-dark-border overflow-hidden">
        <div className="p-6">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">Step 2: Legal Research</h2>
              <p className="text-dark-text-secondary">Simulating review of legal databases, statutes, and case law.</p>
            </div>
          </div>
        ...
        <div className="bg-gray-900/50 px-6 py-4 flex items-center justify-end gap-4">
          {!isResearching ? (
            ...
            >
              <ScaleIcon className="w-5 h-5 animate-spin" />
              Researching...
            </button>
          )}
        </div>
