...
interface TrainingAdaptationViewProps {
  onAdaptationComplete: () => void;
}

const TrainingAdaptationView: React.FC<TrainingAdaptationViewProps> = ({ onAdaptationComplete }) => {
  ...
  useEffect(() => {
    ...
    if (isAdapting && !isComplete) {
      let stepIndex = 0;
      interval = setInterval(() => {
        if (stepIndex < TRAINING_ADAPTATION_STEPS.length) {
          const currentStep = TRAINING_ADAPTATION_STEPS[stepIndex];
      ...
      <div className="bg-dark-card shadow-lg rounded-xl border border-dark-border overflow-hidden">
        <div className="p-6">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">Step 2: Training Plan Adaptation</h2>
              <p className="text-dark-text-secondary">Simulating physiological analysis and plan optimization.</p>
            </div>
          </div>
        ...
        <div className="bg-gray-900/50 px-6 py-4 flex items-center justify-end gap-4">
          {!isAdapting ? (
            ...
            >
              <DumbbellIcon className="w-5 h-5 animate-spin" />
              Adapting...
            </button>
          )}
        </div>
...
export default TrainingAdaptationView;
