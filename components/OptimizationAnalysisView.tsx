...
const OptimizationAnalysisView: React.FC<OptimizationAnalysisViewProps> = ({ onAnalysisComplete }) => {
  ...
  useEffect(() => {
    ...
    if (isAnalyzing && !isComplete) {
      ...
      interval = setInterval(() => {
        if (stepIndex < OPTIMIZATION_ANALYSIS_STEPS.length) {
          const currentStep = OPTIMIZATION_ANALYSIS_STEPS[stepIndex];
      ...
      <div className="bg-dark-card shadow-lg rounded-xl border border-dark-border overflow-hidden">
        <div className="p-6">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">Step 2: Resume Optimization Analysis</h2>
              <p className="text-dark-text-secondary">Simulating analysis against best practices and keywords.</p>
            </div>
          </div>
        ...
        <div className="bg-gray-900/50 px-6 py-4 flex items-center justify-end gap-4">
          {!isAnalyzing ? (
            ...
            >
              <BriefcaseIcon className="w-5 h-5 animate-spin" />
              Analyzing...
            </button>
          )}
        </div>
