...
import { ChatMessage, DocumentSummary } from '../types';
import { ChatBubbleIcon, SendIcon, FileTextIcon } from './shared/IconComponents';
...
const DocumentQaView: React.FC<DocumentQaViewProps> = ({ documentSummary, documentText }) => {
  ...
  useEffect(() => {
    if (documentText) {
        chatRef.current = createDocuBertChatSession(documentText);
        setMessages([
          {
            id: 'init',
            sender: 'bot',
            text: `I have finished indexing the document titled "${documentSummary.title}". Ask me anything about its content.`,
          },
        ]);
    }
  }, [documentText, documentSummary]);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (fullResponse.trim() === '') {
             setMessages(prev => prev.map(msg => msg.id === botMessageId ? { ...msg, text: "I'm not sure how to answer that based on the document. Could you ask in another way?" } : msg));
        }
    } catch (error) {
        console.error("Error sending message:", error);
        setMessages(prev => prev.map(msg => msg.id === botMessageId ? { ...msg, text: 'Sorry, I had trouble processing that request. Please try again.' } : msg));
    } finally {
        setIsThinking(false);
    }
  };
    ...
    <div className="w-full max-w-4xl mx-auto p-4 md:p-6 h-[calc(100vh-120px)] flex flex-col">
      <div className="bg-dark-card shadow-lg rounded-xl border border-dark-border flex flex-col flex-grow">
        <div className="p-4 border-b border-dark-border flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-white">Q&A with DocuBERT</h2>
            <p className="text-dark-text-secondary truncate pr-4">Document: {documentSummary.title}</p>
          </div>
        </div>
            ...
            {messages.map((msg, index) => {
                ...
                return (
                  <div key={msg.id} className={`flex items-end gap-2 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                    {msg.sender === 'bot' && <div className="w-8 h-8 rounded-full bg-accent flex items-center justify-center font-bold text-gray-900 text-sm flex-shrink-0"><FileTextIcon className="w-5 h-5"/></div>}
