...
import { ChatBubbleIcon, SendIcon, GlobeIcon } from './shared/IconComponents';
...
const TravelConciergeView: React.FC<TravelConciergeViewProps> = ({ itinerary }) => {
  ...
  useEffect(() => {
    if (itinerary) {
        chatRef.current = createTravelBertChatSession(itinerary);
        setMessages([
          {
            id: 'init',
            sender: 'bot',
            text: `Wonderful! I have your itinerary for "${itinerary.tripName}" right here. I'm so excited for you! What's the first thing on your mind? Packing? Food? Let's get you ready for an amazing adventure.`,
          },
        ]);
    }
  }, [itinerary]);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (fullResponse.trim() === '') {
             setMessages(prev => prev.map(msg => msg.id === botMessageId ? { ...msg, text: "That's a great question. Let me look into that for you..." } : msg));
        }
    } catch (error) {
        console.error("Error sending message:", error);
        setMessages(prev => prev.map(msg => msg.id === botMessageId ? { ...msg, text: 'Sorry, it seems there is some turbulence on the line. Please try again.' } : msg));
    } finally {
        setIsThinking(false);
    }
  };
    ...
    <div className="w-full max-w-4xl mx-auto p-4 md:p-6 h-[calc(100vh-120px)] flex flex-col">
      <div className="bg-dark-card shadow-lg rounded-xl border border-dark-border flex flex-col flex-grow">
        <div className="p-4 border-b border-dark-border flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-white">Chat with TravelBERT</h2>
            <p className="text-dark-text-secondary truncate pr-4">Trip: {itinerary.tripName}</p>
          </div>
        </div>
            ...
            {messages.map((msg, index) => {
                ...
                return (
                  <div key={msg.id} className={`flex items-end gap-2 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                    {msg.sender === 'bot' && <div className="w-8 h-8 rounded-full bg-accent flex items-center justify-center font-bold text-gray-900 text-sm flex-shrink-0"><GlobeIcon className="w-5 h-5"/></div>}
