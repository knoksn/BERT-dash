...
interface EngineCalibrationViewProps {
  onCalibrationComplete: () => void;
}

const EngineCalibrationView: React.FC<EngineCalibrationViewProps> = ({ onCalibrationComplete }) => {
  ...
  useEffect(() => {
    ...
    if (isTuning && !isComplete) {
      ...
      interval = setInterval(() => {
        if (stepIndex < ENGINE_CALIBRATION_STEPS.length) {
          const currentStep = ENGINE_CALIBRATION_STEPS[stepIndex];
      ...
      <div className="bg-dark-card shadow-lg rounded-xl border border-dark-border overflow-hidden">
        <div className="p-6">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">Step 2: Calibrate Engine</h2>
              <p className="text-dark-text-secondary">Simulate tuning the vehicle's engine for peak performance.</p>
            </div>
          </div>
        ...
        <div className="bg-gray-900/50 px-6 py-4 flex items-center justify-end gap-4">
          {!isTuning ? (
            ...
            >
              <WrenchIcon className="w-5 h-5 animate-spin" />
              Calibrating...
            </button>
          )}
        </div>
...
export default EngineCalibrationView;
