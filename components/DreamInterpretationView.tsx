...
const DreamInterpretationView: React.FC<DreamInterpretationViewProps> = ({ dreamInterpretation }) => {
  ...
  useEffect(() => {
    if (dreamInterpretation) {
        chatRef.current = createDreamBertChatSession(dreamInterpretation);
  ...
  useEffect(() => {
    const sendInitialMessage = async () => {
      if (chatRef.current && !isInitialMessageSent.current && messages.length === 0) {
        ...
        try {
          // Send a simple starter message to trigger the AI's first response with the disclaimer.
          const stream = await chatRef.current.sendMessageStream({ message: `I'm ready to discuss my dream, "${dreamInterpretation.title}".` });
          let fullResponse = '';
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (fullResponse.trim() === '') {
             setMessages(prev => prev.map(msg => msg.id === botMessageId ? { ...msg, text: "That is a fascinating connection. What else comes to mind?" } : msg));
        }
    } catch (error) {
        console.error("Error sending message:", error);
        setMessages(prev => prev.map(msg => msg.id === botMessageId ? { ...msg, text: 'My apologies, the connection to the dream world is hazy. Please try again.' } : msg));
    } finally {
        setIsThinking(false);
    }
  };
