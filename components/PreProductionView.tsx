...
interface PreProductionViewProps {
  onPreProductionComplete: () => void;
}

const PreProductionView: React.FC<PreProductionViewProps> = ({ onPreProductionComplete }) => {
  ...
  useEffect(() => {
    ...
    if (isWorking && !isComplete) {
      ...
      interval = setInterval(() => {
        if (stepIndex < PRE_PRODUCTION_STEPS.length) {
          const currentStep = PRE_PRODUCTION_STEPS[stepIndex];
      ...
      <div className="bg-dark-card shadow-lg rounded-xl border border-dark-border overflow-hidden">
        <div className="p-6">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">Step 2: Pre-Production</h2>
              <p className="text-dark-text-secondary">Simulating checklists, crew coordination, and scheduling.</p>
            </div>
          </div>
        ...
        <div className="bg-gray-900/50 px-6 py-4 flex items-center justify-end gap-4">
          {!isWorking ? (
            ...
            <button
              onClick={onPreProductionComplete}
            ...
            >
              <ClipboardListIcon className="w-5 h-5 animate-spin" />
              Working...
            </button>
          )}
        </div>
