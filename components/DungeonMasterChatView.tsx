...
import { ChatBubbleIcon, SendIcon, ScrollIcon } from './shared/IconComponents';
...
const DungeonMasterChatView: React.FC<DungeonMasterChatViewProps> = ({ quest }) => {
  ...
  useEffect(() => {
    if (quest) {
        chatRef.current = createQuestBertChatSession(quest);
        setMessages([
          {
            id: 'init',
            sender: 'bot',
            text: `Excellent! The adventure "${quest.title}" is laid out before you. How can I, the Dungeon Master, help you refine this tale?`,
          },
        ]);
    }
  }, [quest]);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (fullResponse.trim() === '') {
             setMessages(prev => prev.map(msg => msg.id === botMessageId ? { ...msg, text: "An interesting idea! How could we expand on that?" } : msg));
        }
    } catch (error) {
        console.error("Error sending message:", error);
        setMessages(prev => prev.map(msg => msg.id === botMessageId ? { ...msg, text: 'Sorry, my inspiration seems to have fled. Please try again.' } : msg));
    } finally {
        setIsThinking(false);
    }
  };
    ...
    <div className="w-full max-w-4xl mx-auto p-4 md:p-6 h-[calc(100vh-120px)] flex flex-col">
      <div className="bg-dark-card shadow-lg rounded-xl border border-dark-border flex flex-col flex-grow">
        <div className="p-4 border-b border-dark-border flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-white">Chat with the DM</h2>
            <p className="text-dark-text-secondary truncate pr-4">Quest: {quest.title}</p>
          </div>
        </div>
            ...
            {messages.map((msg, index) => {
                ...
                return (
                  <div key={msg.id} className={`flex items-end gap-2 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                    {msg.sender === 'bot' && <div className="w-8 h-8 rounded-full bg-accent flex items-center justify-center font-bold text-gray-900 text-sm flex-shrink-0"><ScrollIcon className="w-5 h-5"/></div>}
