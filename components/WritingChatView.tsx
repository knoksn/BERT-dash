...
import { ChatMessage, StoryPremise } from '../types';
...
const WritingChatView: React.FC<WritingChatViewProps> = ({ storyPremise }) => {
  ...
  useEffect(() => {
    chatRef.current = createStoryChatSession(storyPremise);
    setMessages([
      {
        id: 'init',
        sender: 'bot',
        text: `The muse is ready! I have the details for "${storyPremise.title}". Let's write this story together. What should happen first?`,
      },
    ]);
  }, [storyPremise]);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
        ...
        setMessages(prev =>
          prev.map(msg =>
            msg.id === botMessageId ? { ...msg, text: 'Sorry, I seem to have writer\'s block! Please try again.' } : msg
          )
        );
    } finally {
        setIsThinking(false);
    ...
    <div className="w-full max-w-4xl mx-auto p-4 md:p-6 h-[calc(100vh-120px)] flex flex-col">
      <div className="bg-dark-card shadow-lg rounded-xl border border-dark-border flex flex-col flex-grow">
        <div className="p-4 border-b border-dark-border flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-white">Write with StoryBERT</h2>
            <p className="text-dark-text-secondary">Title: {storyPremise.title}</p>
          </div>
        </div>
            ...
            {messages.map((msg, index) => {
                ...
                return (
                  <div key={msg.id} className={`flex items-end gap-2 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                    {msg.sender === 'bot' && <div className="w-8 h-8 rounded-full bg-accent flex items-center justify-center font-bold text-gray-900 text-sm flex-shrink-0">SB</div>}
