...
import { ChatMessage, WorkoutPlan } from '../types';
...
const FitnessCoachView: React.FC<FitnessCoachViewProps> = ({ workoutPlan }) => {
  ...
  useEffect(() => {
    if (workoutPlan) {
        chatRef.current = createFitBertChatSession(workoutPlan);
  ...
  useEffect(() => {
    const sendInitialMessage = async () => {
      if (chatRef.current && !isInitialMessageSent.current && messages.length === 0) {
        ...
        try {
          // Send a simple starter message to trigger the AI's first response, which should include the disclaimer.
          const stream = await chatRef.current.sendMessageStream({ message: "Let's get started with my plan!" });
          let fullResponse = '';
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (fullResponse.trim() === '') {
             setMessages(prev => prev.map(msg => msg.id === botMessageId ? { ...msg, text: "That's a great question. Let's break that down..." } : msg));
        }
    } catch (error) {
        console.error("Error sending message:", error);
        setMessages(prev => prev.map(msg => msg.id === botMessageId ? { ...msg, text: 'Sorry, I need to catch my breath! Please try that again.' } : msg));
    } finally {
        setIsThinking(false);
    }
  };
