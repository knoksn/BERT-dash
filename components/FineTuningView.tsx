...
import { RocketIcon } from './shared/IconComponents';

interface FineTuningViewProps {
  onTuningComplete: () => void;
  dataPairCount: number;
}

const FineTuningView: React.FC<FineTuningViewProps> = ({ onTuningComplete, dataPairCount }) => {
  ...
  useEffect(() => {
    ...
    if (isTuning && !isComplete) {
      ...
      interval = setInterval(() => {
        if (stepIndex < FINE_TUNING_STEPS.length) {
          const currentStep = FINE_TUNING_STEPS[stepIndex];
      ...
      <div className="bg-dark-card shadow-lg rounded-xl border border-dark-border overflow-hidden">
        <div className="p-6">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">Step 2: Fine-Tune DarkBERT</h2>
              <p className="text-dark-text-secondary">Simulate the training process with the prepared dataset.</p>
            </div>
          </div>
        ...
        <div className="bg-gray-900/50 px-6 py-4 flex items-center justify-end gap-4">
          {!isTuning ? (
            ...
            >
              <RocketIcon className="w-5 h-5 animate-ping" />
              Tuning in Progress...
            </button>
          )}
        </div>
