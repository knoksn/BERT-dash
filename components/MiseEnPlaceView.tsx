...
const MiseEnPlaceView: React.FC<MiseEnPlaceViewProps> = ({ onPrepComplete }) => {
  ...
  useEffect(() => {
    ...
    if (isPrepping && !isComplete) {
      let stepIndex = 0;
      interval = setInterval(() => {
        if (stepIndex < MISE_EN_PLACE_STEPS.length) {
          const currentStep = MISE_EN_PLACE_STEPS[stepIndex];
      ...
      <div className="bg-dark-card shadow-lg rounded-xl border border-dark-border overflow-hidden">
        <div className="p-6">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">Step 2: Mise en Place</h2>
              <p className="text-dark-text-secondary">Simulating the process of preparing ingredients and equipment.</p>
            </div>
          </div>
        ...
        <div className="bg-gray-900/50 px-6 py-4 flex items-center justify-end gap-4">
          {!isPrepping ? (
            ...
            >
              <ChefHatIcon className="w-5 h-5 animate-spin" />
              Prepping...
            </button>
          )}
        </div>
...
export default MiseEnPlaceView;