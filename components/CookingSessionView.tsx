...
import { ChatMessage, RecipeProfile } from '../types';
import { ChatBubbleIcon, SendIcon, ChefHatIcon } from './shared/IconComponents';
...
const CookingSessionView: React.FC<CookingSessionViewProps> = ({ recipeProfile }) => {
  ...
  useEffect(() => {
    if (recipeProfile) {
        chatRef.current = createRoBertoChatSession(recipeProfile);
        setMessages([
          {
            id: 'init',
            sender: 'bot',
            text: `Magnifico! Mise en place is complete. I'm excited to cook "${recipeProfile.dishName}" with you. I have the recipe right here. Let me know when you're ready for the first step!`,
          },
        ]);
    }
  }, [recipeProfile]);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (fullResponse.trim() === '') {
             setMessages(prev => prev.map(msg => msg.id === botMessageId ? { ...msg, text: "That's a good question! Let me check my notes..." } : msg));
        }
    } catch (error) {
        console.error("Error sending message:", error);
        setMessages(prev => prev.map(msg => msg.id === botMessageId ? { ...msg, text: 'Oops! I think I dropped a spoon. Can you ask again?' } : msg));
    } finally {
        setIsThinking(false);
    }
  };
    ...
    <div className="w-full max-w-4xl mx-auto p-4 md:p-6 h-[calc(100vh-120px)] flex flex-col">
      <div className="bg-dark-card shadow-lg rounded-xl border border-dark-border flex flex-col flex-grow">
        <div className="p-4 border-b border-dark-border flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-white">Cook with RoBERTo</h2>
            <p className="text-dark-text-secondary truncate pr-4">Dish: {recipeProfile.dishName}</p>
          </div>
        </div>
            ...
            {messages.map((msg, index) => {
                ...
                return (
                  <div key={msg.id} className={`flex items-end gap-2 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                    {msg.sender === 'bot' && <div className="w-8 h-8 rounded-full bg-accent flex items-center justify-center font-bold text-gray-900 text-sm flex-shrink-0"><ChefHatIcon className="w-5 h-5"/></div>}
